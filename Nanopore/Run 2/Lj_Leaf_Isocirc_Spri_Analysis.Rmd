---
title: "221024_Leaf_isocirc_Spri_Analysis"
author: "Asa Budnick"
date: "2022-11-09"
...
Full_dataset<-read.csv(file = "LeafSpri_Prep.info",header = F, sep="\t")
...
library(dplyr)
  ...
  Reads_Table<-read.csv(file = "LeafSpri_Prep.reads", sep = "\t", header = T)
str(Reads_Table)
...
library(ggplot2)
library(ShortRead)
cand_circ<-readFasta(dirPath = "LeafSpri_Prep.cand_circ.fa")
...
#one cell is over 32,767 characters which is too much for an excel file - need to find and truncate contents of that cell.. 
for (i in 1:40){
  print(i)
  print(max(nchar(Final_Annotated_Circs2[,i]),na.rm = T))
  print(which.max(nchar(Final_Annotated_Circs2[,i])))
}
#problem is in DB object name column, since most of this information is unnecessary and repetitive for the downstream analyses truncate this column to only the first 1000 characters. 
trunctest<-as.data.frame(str_trunc(Final_Annotated_Circs2[,9], 1000, side="right"))
Final_Annotated_Circs2$`DB Object Name`<-trunctest$`str_trunc(Final_Annotated_Circs2[, 9], 1000, side = "right")`

#worked 
write.xlsx(x=Final_Annotated_Circs2,file = "221024_leaf_spri_Full_Annotated_isoCirc-Prep_CIRI-long-Analsis.xls")
write.table(x = Final_Annotated_Circs2, file = "221024_leaf_spri_Full_Annotated_isoCirc-Prep_CIRI-long-Analsis.tsv",sep = 
            "\t")
