...
Nanopore<-rbind(Leaf_Bluepippin,Leaf_Spri)
...
#Discrepency likely comes from a difference between the CCS and the assigned length which is also a product of CCS but probably an internal CCS that CIRI-long generates and doesn't share 
```
...
#458 circRNAs are filtered, apx 9.8% of nanopore data
...
Nanopore_Run_set<-list(Leaf_Bluepippin=unique(Filtered_simplest$New_ID[Filtered_simplest$Run=="Leaf_Bluepippin"]),Leaf_Spri=unique(Filtered_simplest$New_ID[Filtered_simplest$Run=="Leaf_Spri"]))
...
```{r nanopore vs ciri2 vs clear venn diagram individual runs}
Ind_Datasets_comp<-list(Leaf_Bluepippin=unique(Filtered_simplest$New_ID[Filtered_simplest$Run=="Leaf_Bluepippin"]),Leaf_Spri=unique(Filtered_simplest$New_ID[Filtered_simplest$Run=="Leaf_Spri"]),CircExplorer3=unique(Filtered_simplest$New_ID[Filtered_simplest$Prep=="CLEAR"]),CIRI2=unique(Filtered_simplest$New_ID[Filtered_simplest$Prep=="CIRI2"]))
...
unique(Filtered_simplest$Prep)
Nano_Vs_Illum_set<-list(Nanopore=unique(Filtered_simplest$New_ID[Filtered_simplest$Prep=="CEnLR" ]), Illumina=unique(Filtered_simplest$New_ID[(Filtered_simplest$Prep=="CIRI2" |Filtered_simplest$Prep=="CLEAR")]))
...
Filt_minus_circpan$seq[Filt_minus_circpan$Prep=="CEnLR"]<-"Nanopore"
...
chromplot<-plotKaryotype(genome=lj_chrom,chromosomes =c("")  ,plot.type = 2,plot.params = plotparams)
...
filter_data$Prep[filter_data$Prep=="CLEAR"]<-"CLEAR"
filter_data$Prep<-factor(x=filter_data$Prep,levels = c("CIRI2","CLEAR","CEnLR"))
...
filter_data$status[filter_data$Prep=="CEnLR" & filter_data$circ_id %in% fails_ir]<-"Poor Quality Sequence"
filter_data$status[filter_data$Prep=="CEnLR" & filter_data$circ_id %in% fails_trif]<-"Poor Quality Sequence"
...
sizingdata$Prep<-factor(x=sizingdata$Prep,levels = c("CIRI2","CLEAR","CEnLR"))
...
typedata$Prep<-factor(x=typedata$Prep,levels = c("CIRI2","CLEAR","CEnLR"))
...
CircRNA_Set<-list(Nanopore=unique(Filtered_simplest$New_ID[Filtered_simplest$Prep=="CEnLR" ]), CIRI2=unique(Filtered_simplest$New_ID[Filtered_simplest$Prep=="CIRI2"]), CLEAR=unique(Filtered_simplest$New_ID[Filtered_simplest$Prep=="CLEAR"]))
...
Gene_Prep_Comp<-list(Nanopore=unique(Full_data_size_and_type$gene_id[Full_data_size_and_type$Prep =="CEnLR"]), CIRI2=unique(Full_data_size_and_type$gene_id[Full_data_size_and_type$Prep =="CIRI2"]),CLEAR=unique(Full_data_size_and_type$gene_id[Full_data_size_and_type$Prep =="CLEAR"]))
...
Size_table<-Filtered_simplest %>%  
  group_by(Prep) %>% 
  summarize(Med= median(circ_len), Average = mean(circ_len), Standard_Deviation= sd(circ_len))
Size_table2<-Filtered_simplest %>%  
  group_by(Run) %>% 
  summarize(Med= median(circ_len), Average = mean(circ_len), Standard_Deviation= sd(circ_len))

Size_table3<-Filtered_simplest %>%  
  group_by(Prep, replicate) %>% 
  summarize(Med= median(circ_len), Average = mean(circ_len), Standard_Deviation= sd(circ_len))
...
combined_filtered$seq[combined_filtered$Prep=="CEnLR"]<-"Nanopore"
...
```{r pull only the 6 that are in all 3 }
...
Nanopore_2xReps<-combined_filtered[combined_filtered$Prep=="CEnLR",] %>% 
  group_by(Cluster_ID) %>% 
  filter(n_distinct(Run) >=2)

#add in the information from the other sources where these circRNAs are found 
Nanopore_2xReps<-rbind(Nanopore_2xReps, combined_filtered[combined_filtered$Prep!="CEnLR" & combined_filtered$Cluster_ID %in% Nanopore_2xReps$Cluster_ID,])
...
```{r export in final supplemental file formats}
#Supp2 Input Data 
xlsx::write.xlsx(x=Leaf_Bluepippin, file = "Supplementary Table S2.xlsx", sheetName = "Nanopore_BluePippin")
xlsx::write.xlsx(x=Leaf_Bluepippin, file = "Supplementary Table S2.xlsx", sheetName = "Nanopore_SPRI",append = T)
xlsx::write.xlsx(x=CIRI2, file="Supplementary Table S2.xlsx",sheetName = "CIRI2", append=T)
xlsx::write.xlsx(x=CX3, file="Supplementary Table S2.xlsx",sheetName = "CLEAR", append=T)
xlsx::write.xlsx(x=CircPANEL, file="Supplementary Table S2.xlsx",sheetName = "circPanel-LRS",append = T)

#Supp3 Output Data

Supp3List<-list("Full_Filtered_Results" = combined_filtered,"CIRI2_Found_in_All_Reps" = CIRI2_allReps, "CLEAR_Found_in_All_Reps"= CLEAR_allReps, "Nanopore_Found_in_both_runs" =  Nanopore_2xReps)
openxlsx::write.xlsx(x=Supp3List, file = "Supplementary Table S3.xlsx")


#Supp 4 
Supp4list<-list("CEnLR_and_SR_Any_Overlap"=illum_nano_cons2, "CEnLR_and_both_SR"=illum_nano_cons3)
openxlsx::write.xlsx(x=Supp4list, file = "Supplementary Table S4.xlsx")
...
```{r compare the number of reads found across each of the methods for the 15 circRNAs that are found in all}
...
```{r kill knit prior to crash from large blast vector}
...
Filtered_with_blast$ortho_org<-substr(x=Filtered_with_blast$ortholog_circ_id, start=1, stop=2)
unique(Filtered_with_blast$ortho_org)
...
length(unique(Found_in_bean$gene_id))
...
```{r Reperform Ortho analysis focusing on 12 PCR confirmed circRNAs rather than the overlap set}
confirmed_circs<-c("LjG1.1_chr1:134081453-134082655,LjG1.1_chr1:134081452-134082655",
"LjG1.1_chr1:65260397-65261985,LjG1.1_chr1:65260396-65261985",
"LjG1.1_chr3:92618233-92618658,LjG1.1_chr3:92618232-92618658",
"LjG1.1_chr6:38896647-38897422,LjG1.1_chr6:38896647-38897426", 
"LjG1.1_chr1:16045704-16046862,LjG1.1_chr1:16045704-16046857", 
"LjG1.1_chr2:92355020-92356275,LjG1.1_chr2:92355020-92356270", 
"LjG1.1_chr5:18494229-18496352,LjG1.1_chr5:18494228-18496352",
"LjG1.1_chr3:76765996-76766562",
"LjG1.1_chr1:93054412-93054854",
"LjG1.1_chr1:34194372-34194761,LjG1.1_chr1:34194371-34194761",
"LjG1.1_chr1:18146757-18147201,LjG1.1_chr1:18146756-18147201", 
"LjG1.1_chr1:122594070-122598565,LjG1.1_chr1:122594069-122598565") 

confirmed_circs %in% combined_filtered$Cluster_ID


Confirmed_table<-combined_filtered[combined_filtered$Cluster_ID %in% confirmed_circs,]
At_circs2<-At_Circs[,c(1:8)]
str(At_circs2)
colnames(At_circs2)<-c(colnames(At_circs2)[1:7], "Parent_gene")
#Compare to Ath and Phvu
At_conserved2<-merge(Confirmed_table,At_circs2, by.x="atg", by.y="Parent_gene", all=F)

length(unique(At_conserved2[,8]))
Found_in_bean2<-merge(Confirmed_table,wu2020_bean_circRNAs, by.x="atg", by.y="Arabidopsis.Best.Aligned.ID", all=F)
length(unique(Found_in_bean2$Cluster_ID))
length(unique(Found_in_bean2$gene_id))


AtgUniqes<-data_frame(unique(At_conserved2[,c(1,7,8,43)]))
PhvuUniqes<-data_frame(unique(Found_in_bean2[,c(1,7,8,45,43)]))

Atg_Circs_per_gene<- AtgUniqes %>% 
  group_by(atg,gene_id,GFF3_Annotation) %>% 
  summarize(AtCircs=n_distinct(CircRNA_ID))

Phvu_circs_per_gene<- PhvuUniqes %>% 
  group_by(atg,gene_id,GFF3_Annotation,Parental.Gene) %>% 
  summarize(PvCircs=n_distinct(CircRNA.ID))

At_pv_merg<-merge(x=Atg_Circs_per_gene, Phvu_circs_per_gene, by=c("atg", "gene_id"),all=T)

#import the Gm and Os circs. These were generated by id'ing the corresponding gene IDs through blasting the lotus gene protein sequence against Gm and Os and then searching those gene ids in the corresponding plant circ base datasets. 
Gm_os_gene_circs<-read.csv(file="GM_os_circ_gene_ortho.csv", header=T)

Gene_ortho_merge<-merge(x=At_pv_merg, y=Gm_os_gene_circs, by="gene_id", all=T)
#remove rows with NA in Lotus gene id
Gene_ortho_merge<-Gene_ortho_merge[Gene_ortho_merge$gene_id!="",]

openxlsx::write.xlsx(x=Gene_ortho_merge,file = "CircRNA_Gene_Orthologs.xlsx")

#blast confirmed sequence set 
Confirmed_table2<-Confirmed_table[!is.na(Confirmed_table$CIRILong_Circular_Consensus_Sequence),]
CCS<-DNAStringSet(x=Confirmed_table2$CIRILong_Circular_Consensus_Sequence,use.names = T)
names(Conf_CCS)<-Confirmed_table2$Cluster_ID

Conf_v_all_full_circ<-predict(object = At_Gm_Os_bldb,newdata = Conf_CCS)

SR_seqs<-read.fasta(file = "SR_circ_seqs.fasta")
DNAStringSet()

SR_seqs<-readDNAStringSet(file = "SR_circ_seqs.fasta")
SR_Conf_V_all_Full_circ<-predict(object = At_Gm_Os_bldb, newdata=SR_seqs)
```
...
```{r table of chloroplastic gene ID circRNA counts}
Chloroplastic<-combined_filtered[combined_filtered[,2]=="LjG1.1_chloro",]

Chloroplastic_summary<- Chloroplastic %>%  
  group_by(gene_id, seq) %>%
  summarize(Unique_CircRNAs<-n_distinct(Cluster_ID))
  
```
```{r karyoplot for chloroplast}
#bring in GAF and chloroplastic genes 
Genes_GTF<-read.csv(file = "../Reference_Genomes/Lotus/Lotusjaponicus_Gifu_v1.3_predictedProteins.agat.convert.gtf", sep = "\t", skip = 9)
Genes_GTF<-separate(data=Genes_GTF, col=X.7, into= c("GeneID","ID","Name"), sep=";")
Genes_GTF$GeneID<-gsub(x = Genes_GTF$GeneID, pattern = "gene_id ", replacement="")

Genes_GTF<-Genes_GTF[Genes_GTF$X.1=="gene",]
Genes_ForGR<-unique(data.frame(chrom=Genes_GTF$X..sequence.region...LjG1.1_mito.1.360143, start=as.numeric(Genes_GTF$X.2), end=as.numeric(Genes_GTF$X.3),cate=Genes_GTF$GeneID))

Genes_Granges<-toGRanges(Genes_ForGR)

chlorcircs<-unique(data.frame(chrom=Chloroplastic$chrom,start=as.numeric(Chloroplastic$start),end=as.numeric(Chloroplastic$end),cate=Chloroplastic$seq))

chlorcircs_granges<-toGRanges(chlorcircs)

#pull gene positions for the highest circ counts 
Genes_with_high_count<-c("LotjaGiC1v0012100", "LotjaGiC1v0006900","LotjaGiC1v0011800", "LotjaGiC1v0009200","LotjaGiC1v0009500","LotjaGiC1v0000200","LotjaGiC1v0012500","LotjaGiC1v0000500","LotjaGiC1v0008800","LotjaGiC1v0012700")

Gene_markers_df<-Genes_ForGR[Genes_ForGR$cate %in% Genes_with_high_count,-3]
colnames(Gene_markers_df)<-c("chr","pos","labels")
#plotting 
plotparams<-getDefaultPlotParams(plot.type = 2)
plotparams$leftmargin <- 0.2
plotparams$ideogramheight<-20
plotparams$topmargin<-200

kp<-plotKaryotype(genome=lj_chrom,chromosomes = "LjG1.1_chloro", plot.type = 2,plot.params = plotparams)
kpAddBaseNumbers(kp)
kpPlotRegions(kp, data=chlorcircs_granges, data.panel = 1, col= "red")
kpPlotMarkers(kp, chr=Gene_markers_df$chr, x=Gene_markers_df$pos, labels=Gene_markers_df$labels)
kpPlotRegions(kp, data = Genes_Granges,data.panel = 2,col="grey")

```

```{r merge in positions to start and stops}
Chloroplastic_summary<-merge(Chloroplastic_summary,Genes_ForGR[,-1], by.x="gene_id", by.y="cate")
openxlsx::write.xlsx(x=Chloroplastic_summary,file = "Chloroplastic_circRNA_genes.xlsx")
```


```{r More calculations }
#CEnLR number of unique nuclear circRNAs (and associated Gene IDs)
CEnLR<-combined_filtered[combined_filtered$seq=="Nanopore",]
CEnLR_nuc<-CEnLR[!(CEnLR$chrom %in% c("LjG1.1_chloro","LjG1.1_chr0","LjG1.1_mito")),]
CEnLR_nuc_genic<-CEnLR_nuc[CEnLR_nuc$gene_id!="",]
CEnLR_nuc_genic<-CEnLR_nuc_genic[!CEnLR_nuc_genic$GFF3_Annotation=="",]
n_distinct(CEnLR_nuc_genic$Cluster_ID, na.rm=T)
n_distinct(CEnLR_nuc_genic$gene_id, na.rm=T)
n_distinct(CEnLR_nuc_genic$atg, na.rm=T)


#SR
SR<-combined_filtered[combined_filtered$seq=="Illumina",]
SR_nuc<-SR[!(SR$chrom %in% c("LjG1.1_chloro","LjG1.1_chr0","LjG1.1_mito")),]
SR_nuc_genic<-SR_nuc[SR_nuc$gene_id!="",]
SR_nuc_genic<-SR_nuc_genic[!SR_nuc_genic$GFF3_Annotation=="",]
n_distinct(SR_nuc_genic$Cluster_ID, na.rm=T)
n_distinct(SR_nuc_genic$gene_id, na.rm=T)
n_distinct(SR_nuc_genic$atg, na.rm=T)

Gene_lists_R<-list("CEnLR_Unique_Nuclear_Genic" = unique(CEnLR_nuc_genic$atg,na.rm=T),"SR_Unique_Nuclear_Genic" = unique(SR_nuc_genic$atg,na.rm=T))
openxlsx::write.xlsx(x=Gene_lists_R, file = "R_unique_nuclear_gene_lists.xlsx")


#Calculate nuclear intergenic, chloroplast, and mitocondrial circs for CEnLR and SR
n_distinct(CEnLR_nuc$Cluster_ID[CEnLR_nuc$circRNA_type=="intergenic"],na.rm=T)
n_distinct(CEnLR$Cluster_ID[CEnLR$chrom=="LjG1.1_chloro"],na.rm=T)
n_distinct(CEnLR$Cluster_ID[CEnLR$chrom=="LjG1.1_mito"],na.rm=T)


n_distinct(SR_nuc$Cluster_ID[SR_nuc$circRNA_type=="intergenic_region"],na.rm=T)
n_distinct(SR$Cluster_ID[SR$chrom=="LjG1.1_chloro"],na.rm=T)
n_distinct(SR$Cluster_ID[SR$chrom=="LjG1.1_mito"],na.rm=T)
