---
title: "230619_Circpanel2_allreads_analysis"
author: "Asa Budnick"
date: "2023-06-19"
...
Full_dataset<-read.csv(file = "Panel_test.info",header = F, sep="\t")
...
#GAF file was originally obtained from LotusBase.com 
...
library(dplyr)
  ...
  Reads_Table<-read.csv(file = "Panel_test.reads", sep = "\t", header = T)
str(Reads_Table)
...
cand_circ<-readFasta(dirPath = "Panel_test.cand_circ.fa")
...
write.xlsx(x=Final_Annotated_Circs2,file = "CircPanelLRS_Cirilong_Results_annotated.xls")

write.table(x = Final_Annotated_Circs2, file = "CircPanelLRS_Cirilong_Results_annotated.tsv",sep = 
            "\t")
```
...
```{r make fasta files for each locus to add to benchling}

#make table of circRNAs by ID + geneID + DB object name + Sequence 
fastaform_DF<-unique(Final_Annotated_Circs2[,c(1,4,40,39)])
fastaform_DF$description<-str_split(fastaform_DF$GFF_Anno, pattern =";",simplify = T)[,1]

colnames(fastaform_DF)<-c("name","geneID","full_anno","seq","description")
...
writeFasta<-function(data, filename){
  ...
  for (rowNum in 1:nrow(data)){
    fastaLines = c(fastaLines, as.character(paste(">", data[rowNum,"name"]," ",                                            data[rowNum,"description"], sep = "")))
...
dflist<-split(fastaform_DF,f=fastaform_DF$geneID)
#names(dflist)<-unique(fastaform_DF$geneID)
for (i in 1:length(dflist)){ 
  print(i)
  writeFasta(data=dflist[[i]],filename= paste(trimws(names(dflist)[i]),"_",gsub(x=gsub(x=dflist[[i]]$description[1],pattern = "/", replacement = "_"), pattern = " ", replacement = "_"),".fasta",sep = "")) 
}
...
unique(Final_Annotated_Circs2$circ_id)
#unique(Final_Annotated_Circs$`DB Object Name`)
unique(str_split(unique(Final_Annotated_Circs2$GFF_Anno), pattern =";",simplify = T)[,1])

unique(Final_Annotated_Circs2$circ_id[which(Final_Annotated_Circs2$gene_id == "LotjaGi1g1v0664300")])

```


```{r compare score across length}
score_length_scatter<-ggplot(Final_Annotated_Circs2, aes(x=Final_Annotated_Circs2$circ_len, y=Final_Annotated_Circs2$score))+
  geom_point()
score_length_scatter
```
